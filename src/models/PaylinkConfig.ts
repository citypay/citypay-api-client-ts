...
import {
    ...
    PaylinkFieldGuardModelToJSONTyped,
...
import {
    ...
    PaylinkPartPaymentsToJSONTyped,
...
export interface PaylinkConfig {
     ...
     * 
     * If you request an iframe mode and the browser width is deemed as being small (< 768px) then an inline mode will be enforced. This is to ensure that mobile users have an appropriate user experience.
     * 
     * The default type if not supplied is **iframe**.
     * 
     * @type {string}
    ...
    descriptor?: string;
    /**
     * Specifies a period of time in seconds after which the token cannot be used. A value of 0 defines that the token will never expire. The API will convert an expiry time based on a string value.
     * 
     * For instance:
...
export function PaylinkConfigToJSON(json: any): PaylinkConfig {
    return PaylinkConfigToJSONTyped(json, false);
}

export function PaylinkConfigToJSONTyped(value?: PaylinkConfig | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
